...
function proposals(id){
 ...
 const action=item.link?ext(D.links[item.link],item.action,'button'):item.href?internal(item.href,item.action,'button'):'';
 const note=item.note?`<p class="small">${escape(item.note)}</p>`:'';
 return intro('propuestas')+`<div class="wrap page-body">${subnav(D.proposals,'propuestas',item.id,'subnav','Tipos de propuestas')}<article class="proposal"><div class="proposal-copy">${eyebrow(item.name)}<h2>${escape(item.title)}</h2><p>${escape(item.description)}</p>${action}${note}${projects}</div><figure class="proposal-art"><img src="assets/${image}" alt="Encuentros y prácticas compartidas, archivo audiovisual de Nueva Semilla" width="960" height="540"></figure></article><p class="photo-credit">Fotografías del archivo de los 20 años de Nueva Semilla.</p></div>`;
}
