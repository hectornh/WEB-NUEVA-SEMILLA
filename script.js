...
const names = {'asesoramiento':'Asesoramiento','propuestas':'Propuestas','gastronomia':'Gastronomía','mapa':'Mapa','quienes-somos':'Quiénes Somos'};
const intros = {
  ...
  gastronomia:['Del campo a la mesa','Alimentos con origen, personas que producen y experiencias para volver a encontrarnos alrededor de la mesa.'],
...
function home(){
 ...
 ['asesoramiento','Asesoramiento','Saberes para acompañar tu proyecto.'],
 ['propuestas','Propuestas','Aprender haciendo, junto a otros.'],
 ['gastronomia','Gastronomía','Alimentos y encuentros con origen.'],
 ...
 <article class="journal-card"><div class="media-cover book-cover" aria-hidden="true"><span>SABERES DEL TERRITORIO</span><strong>Agroecología<br>a la Carta</strong><span>LECTURAS COMPARTIDAS</span></div><small>BIBLIOGRAFÍA · 2022</small><h3>Una historia<br>que queda escrita.</h3><p>El recorrido de Nueva Semilla en el capítulo de Darío Colaneri.</p>${internal('#quienes-somos/bibliografia','Conocer el libro')}</article>
...
function gastronomy(id,detail){
 ...
 if(active==='venta-directa'){
  ...
  body+=item?`<article class="detail-intro product-detail">${eyebrow(item.producer)}<h2>${escape(item.name)}</h2><p class="description">${escape(item.description)}</p><p class="notice">Disponibilidad, presentaciones, precios y modalidades de entrega por confirmar con el emprendimiento.</p><div class="actions">${item.participant?internal('#quienes-somos/participantes/'+item.participant,'Conocer el emprendimiento'):internal('#asesoramiento/fungicultura','Conocer a Carolina')}${participantExternal(item.participant)}${internal('#quienes-somos/contacto','Contacto')}</div></article>`:`<div class="product-grid">${D.products.map(p=>`<a href="#gastronomia/venta-directa/${p.id}" class="product-card"><div class="product-icon" aria-hidden="true">${p.symbol}</div><h3>${escape(p.name)}</h3><p>${escape(p.producer)}</p><span class="text-link">Conocer el producto <span aria-hidden="true">↗</span></span></a>`).join('')}</div>`;
 }
 ...
 return intro('gastronomia')+`<div class="wrap page-body">${subnav(D.gastronomy,'gastronomia',active,'subnav','Secciones de gastronomía')}${body}</div>`;
}
function map(){
...
function about(id,detail){
 ...
 if(active==='participantes'){
  ...
  body=`<div class="participant-layout">${subnav(D.participants,'quienes-somos/participantes',p.id,'side-nav','Emprendimientos y participantes')}<article class="participant-panel">${eyebrow(p.location)}<h2>${escape(p.name)}</h2><p>${escape(p.description)}</p>${p.people?people(p.people):''}<div class="actions">${participantLinks(p)}</div><p class="notice">${notice}</p></article></div>${detail?'':photoArchive()}`;
 }
